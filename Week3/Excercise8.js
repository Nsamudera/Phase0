...
5. Once every integer is comapared and the "bigger number" has been determined, find the integer to the right of the "bigger number"
6.display the "bigger number" and the adjacent number.
**/
function pasanganTerbesar(num) {
var numtostring = num.toString() //to convert the number to string, so that we can used the index to compare eacah integer
var biggestnum = numtostring[0] // the biggest number of the inout will be stored here for comparison
...
for(index=0 ; index<numtostring.length ; index++){
  // if the current index number is bigger than the variable 'biggestnum' (which is originally set as the frist number in the input) . . . 
...
return biggestnum+numtostring[biggestnumindex+1]
}
